...
void LaunchController::decideAccount()
{
    ...
    if (accounts->count() == 0) {
        auto offlineAccount = std::make_shared<MinecraftAccount>();
        offlineAccount->setAccountType(AccountType::Offline);
        offlineAccount->setUsername("Player");
        offlineAccount->setProfileName("Player");
        offlineAccount->setUUID(MinecraftAccount::uuidFromUsername("Player"));
        offlineAccount->setAccessToken("offline");
        offlineAccount->setSessionId("offline");
        offlineAccount->setAccountState(AccountState::Online);
        accounts->addAccount(offlineAccount);
        accounts->setDefaultAccount(offlineAccount);
    }
